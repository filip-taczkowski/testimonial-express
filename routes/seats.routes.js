...
router.route('/seats').post((req, res) => {
  ...
  const seatTaken = db.seats.some(item => newSeat.seat == item.seat && newSeat.day == item.day);

  if (seatTaken) {
    res.status(403).json({ message: 'This seast is already taken'});
  } else {
    res.json(db.seats.push(newSeat));
